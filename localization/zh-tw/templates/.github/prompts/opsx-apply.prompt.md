...
**輸入**：可在 `/opsx:apply` 後選配指定變更名稱（例如 `/opsx:apply add-auth`）。如果省略，請檢查是否可以從對話內容中推斷。如果模糊不清或不明確，您必須提示使用者選擇可用的變更。
...
3. **取得實作 (apply) 指令**
   ...
   **處理狀態：**
   - 如果 `state: "blocked"`（缺失 artifacts）：顯示訊息，建議使用 `/opsx:continue`
...
所有任務皆已完成！您可以使用 `/opsx:archive` 封存此變更。
```
