---
description: 使用實驗性 artifact 工作流 (OPSX) 開始新變更
---

使用實驗性 artifact 驅動方法開始新變更。

**輸入**：`/opsx:new` 後的參數為變更名稱（kebab-case），或者是對使用者想要建構內容的描述。

**步驟**

1. **如果未提供輸入，詢問他們想要建構什麼**
...
5. **取得第一個 artifact 的指令**
   第一個 artifact 取決於 schema。檢查狀態輸出以找到第一個狀態為 "ready" 的 artifact。
   ```bash
...
- 第一個 artifact 的模板
- 提示：「準備好建立第一個 artifact 了嗎？執行 `/opsx:continue`，或者直接描述此變更的內容，我會為您草擬。」

**Guardrails**
- 尚未建立任何 artifacts - 僅顯示指令
- 除了顯示第一個 artifact 模板外，不要再進一步操作
- 如果名稱無效（不是 kebab-case），要求提供有效名稱
- 如果已存在同名變更，建議改用 `/opsx:continue`
