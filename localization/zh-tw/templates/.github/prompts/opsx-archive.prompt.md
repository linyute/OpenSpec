...
**輸入**：可在 `/opsx:archive` 後選配指定變更名稱（例如 `/opsx:archive add-auth`）。如果省略，請檢查是否可以從對話內容中推斷。如果模糊不清或不明確，您必須提示使用者選擇可用的變更。
...
4. **評估 delta spec 同步狀態**
   ...
   **提示選項：**
   ...
   如果使用者選擇同步，執行 `/opsx:sync` 邏輯。無論選擇為何，之後皆繼續進行封存。
...
6. **顯示摘要**
   ...
   - 所使用的 schema
   - 封存位置
   - Spec 同步狀態（已同步 / 已跳過同步 / 無 delta specs）
...
**警告：**
- 封存時有 2 個未完成的 artifacts
- 封存時有 3 個未完成的任務
- 已跳過 delta spec 同步（使用者選擇跳過）
